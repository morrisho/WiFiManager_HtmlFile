...
boolean WiFiManager::setupConfigPortal()
{
  ...
  #ifdef USE_EXTERN_HTML
  server->on(String(F("/style.css")).c_str(), std::bind(&WiFiManager::handleCSS, this));
  if (!SPIFFS.begin())
  {
...
void WiFiManager::handleRoot()
{
  ...
  // page.replace("{variable_apName}", _apName);
...
{
  String page;
#ifndef USE_EXTERN_HTML
  page = FPSTR(HTTP_HEADER);
  page.replace("{v}", "Config ESP");
  page += FPSTR(HTTP_SCRIPT);
  page += FPSTR(HTTP_STYLE);
  page += _customHeadElement;
  page += FPSTR(HTTP_HEADER_END); // after this
#endif
  if (scan)
  {
  ...
  file.close();
  tmp.replace("{Variable_wifiPage_content}", page.c_str());
  page = tmp;
#else
  page += FPSTR(HTTP_END); // end here
#endif
  server->sendHeader("Content-Length", String(page.length()));
  server->send(200, "text/html", page);

  Serial.printf("%s", page.c_str());
